...
function UpdateTrainReservation() {
  ...
  const retrieveTrainDetails = () => {
    axios.get(`/api/trains/${id}`).then((res) => {
      if (res.status === 200) {
  ...
  const removeStoppingStation = (index) => {
    ...
    setState((prevState) => ({ ...prevState, stoppingStations: updatedStoppingStations }));
  };
              ...
              >
                <div className="card-body">
                  ...
                  <form className="needs-validation form" noValidate>
                    ...
                    <div className="form-group" style={{ marginBottom: "15px" }}>
  <label style={{ marginBottom: "5px" }}>
    Stopping Station (Format: Station Name, Arrival Time, Departure Time):
  </label>
  <div className="row">
    <div className="col-4">
      <select
